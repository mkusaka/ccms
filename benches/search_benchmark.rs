...
use claude_search::{SearchEngine, SearchOptions, parse_query};
...
fn benchmark_json_parsing(c: &mut Criterion) {
    ...
    c.bench_function("json_parse_single", |b| {
        b.iter(|| {
            ...
            let _: claude_search::SessionMessage = simd_json::serde::from_slice(&mut bytes).unwrap();
        });
    });
}
...
fn benchmark_query_parsing(c: &mut Criterion) {
    c.bench_function("parse_simple_query", |b| {
        b.iter(|| {
            parse_query(black_box("\"hello world\"")).unwrap()
        });
    });
    
    c.bench_function("parse_complex_query", |b| {
        b.iter(|| {
            parse_query(black_box("(hello OR world) AND NOT /test/i")).unwrap()
        });
    });
}
