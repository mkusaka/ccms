...
fn test_session_viewer_quit() {
    ...
    // Test Esc with empty query goes back to ResultDetail
    search.mode = Mode::SessionViewer;
...
fn test_session_viewer_case_insensitive_search() {
        ...
        // Messages with mixed case content
        search.session_messages = vec![
            ...
            r#"{"type":"user","message":{"role":"user","content":"goodbye world"},"uuid":"3","timestamp":"2024-01-01T00:00:02Z","sessionId":"test-session"}"#.to_string(),
        ];
        ...
        // Should find both "Hello WORLD" and "HeLLo there" (case insensitive)
        assert!(find_text_in_buffer(buffer, "Hello WORLD").is_some());
        assert!(find_text_in_buffer(buffer, "HeLLo there").is_some());
    }
    ...
    fn test_session_viewer_filtered_count_display() {
        ...
        search.session_messages = vec![
            ...
            r#"{"type":"assistant","message":{"role":"assistant","content":"Fourth message"},"uuid":"4","timestamp":"2024-01-01T00:00:03Z","sessionId":"test-session"}"#.to_string(),
        ];
        ...
        // Should show "Messages (4 total)" when no filter
        let title = "Messages (4 total)";
        assert!(find_text_in_buffer(buffer, title).is_some());
        
        // Now test with filter
        search.session_query = "First".to_string();
        terminal.draw(|f| search.draw_session_viewer(f)).unwrap();
        let buffer = terminal.backend().buffer();
        
        // Should show filtered count (only 1 message matches "First")
        let title = "Messages (4 total, 1 filtered)";
        assert!(find_text_in_buffer(buffer, title).is_some());
    }
