            ...
            View(
                ...
                Text(
                    content: if has_filter {
                        format!("Messages ({total_messages} total, {filtered_count} filtered)")
                    } else {
                        format!("Messages ({total_messages} total)")
                    },
                    weight: Weight::Bold
                )
            }
                ...
                View(flex_direction: FlexDirection::Column) {
                    #(props.session_state.filtered_indices.iter()
                        ...
                        .map(|(list_idx, &msg_idx)| {
                            let default_msg = "Error: Invalid message index".to_string();
                            let msg = props.session_state.messages.get(msg_idx).unwrap_or(&default_msg);
